...
{
    return GetBoolArg("-testnet", false) ? 18888 : 8888;
}
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: ZCoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use zcoind";
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "listtransactions2"      && n > 1) ConvertTo<boost::int64_t>(params[1]);
    if (strMethod == "listtransactions2"      && n > 2) ConvertTo<boost::int64_t>(params[2]);
